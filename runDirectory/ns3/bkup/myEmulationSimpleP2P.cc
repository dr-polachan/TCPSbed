...
int main (int argc, char *argv[])
{
  ...
  // KPOL - Set default values to command line arguments
        ...
        double varErrorRate = 0.5; //error rate in percentage
  ...
  // KPOL - Assigning ip address to every device [this step creates interfaces from devices]
        ...
        ipv4Address4.SetBase("10.1.200.0","255.255.255.0", "0.0.0.1"); // args = network,mask,base
  ...
  //KPOL - enabling routing protocols
        ...
        staticRoutingA->AddNetworkRouteTo (Ipv4Address ("10.1.200.0"), Ipv4Mask ("255.255.255.0"), 1);
        ...
        staticRoutingB->AddNetworkRouteTo (Ipv4Address ("10.1.200.0"), Ipv4Mask ("255.255.255.0"), 2);
        ...
        staticRoutingC->AddNetworkRouteTo (Ipv4Address ("10.1.200.0"), Ipv4Mask ("255.255.255.0"), 2);
  ...
  //KPOL - Linking rate error model to p2p device in node-2
        ...
        if( varErrorModel.compare("RateErrorModel") == 0 )
        {
           ...
           devices_p2p_2.Get (0)->SetAttribute ("ReceiveErrorModel", PointerValue (em));
        }
